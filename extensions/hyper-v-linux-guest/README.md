...
**In this repo (bldr / CI):**

```bash
make hyper-v-linux-guest PLATFORM=linux/amd64
```

`manifest.yaml` is generated from `manifest.yaml.tmpl` and `vars.yaml` at build time (Sidero bldr convention).

**Standalone (Docker only):**

```bash
chmod +x scripts/assemble-extension-image.sh
VERSION=0.1.0 IMAGE=ghcr.io/windsorcli/hyper-v-linux-guest:0.1.0 \
  ./scripts/assemble-extension-image.sh
```

Sources are compiled from the pinned Linux `tools/hv/` tree (`LINUX_TOOLS_TAG` in `pkg.yaml` / `Dockerfile`).
